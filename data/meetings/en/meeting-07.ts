    ...
    // =============================================
    {
      ...
      sections: [
        ...
        },
        {
          ...
          title: 'The 8px grid — visually',
          description: 'Every cell below = 8px of real width. Any spacing snaps to this cell.',
          total: 12,
          rows: [
            { label: 'Base cell 8px ×12', spans: Array.from({ length: 12 }, () => ({ span: 1, tone: 'muted' as const, label: '8' })) },
            { label: 'Padding 16px = 2 cells', spans: [{ span: 2, tone: 'primary' as const, label: '16' }, { span: 10, tone: 'muted' as const, label: '·' }] },
            { label: 'Padding 24px = 3 cells', spans: [{ span: 3, tone: 'primary' as const, label: '24' }, { span: 9, tone: 'muted' as const, label: '·' }] },
            { label: 'Section gap 48px = 6 cells', spans: [{ span: 6, tone: 'primary' as const, label: '48' }, { span: 6, tone: 'muted' as const, label: '·' }] },
          ],
        },
        {
          type: 'spacing-scale-showcase' as const,
          title: 'Spacing scale in the 8px system',
          description: 'All values are multiples of 8 (with a 4px tolerance for small elements)',
          baseUnit: 8,
          steps: [
            { px: 4, rem: '0.25rem', token: 'p-1', usage: 'minimum gap, icon padding' },
            { px: 8, rem: '0.5rem', token: 'p-2', usage: 'compact padding, gap between small elements' },
            { px: 16, rem: '1rem', token: 'p-4', usage: 'standard card padding, gap in lists' },
            { px: 24, rem: '1.5rem', token: 'p-6', usage: 'section padding, gap between groups' },
            { px: 32, rem: '2rem', token: 'p-8', usage: 'spacing between sections' },
            { px: 48, rem: '3rem', token: 'p-12', usage: 'container padding, gap between large blocks' },
            { px: 64, rem: '4rem', token: 'p-16', usage: 'vertical rhythm between page sections' },
            { px: 96, rem: '6rem', token: 'p-24', usage: 'hero padding, large vertical spacing' },
          ],
        },
        {
          type: 'before-after' as const,
        ...
        },
        {
          ...
          description: '12 divides by 2, 3, 4 and 6 — hence maximum flexibility',
          total: 12,
          rows: [
            { label: '12 × 1', note: 'rarely used', spans: Array.from({ length: 12 }, () => ({ span: 1 })) },
            { label: '6 + 6', note: 'two-column 50/50', spans: [{ span: 6 }, { span: 6 }] },
            { label: '4 + 8', note: 'sidebar + content', spans: [{ span: 4 }, { span: 8 }] },
            { label: '3 + 9', note: 'narrow sidebar + wide content', spans: [{ span: 3 }, { span: 9 }] },
            { label: '4 + 4 + 4', note: 'three equal columns', spans: [{ span: 4 }, { span: 4 }, { span: 4 }] },
            { label: '3 + 3 + 3 + 3', note: 'four equal columns', spans: [{ span: 3 }, { span: 3 }, { span: 3 }, { span: 3 }] },
            { label: '2 + 8 + 2', note: 'centered content with margins', spans: [{ span: 2, tone: 'muted' as const }, { span: 8, tone: 'primary' as const }, { span: 2, tone: 'muted' as const }] },
            { label: '3 + 6 + 3', note: 'classic centered layout', spans: [{ span: 3, tone: 'muted' as const }, { span: 6, tone: 'primary' as const }, { span: 3, tone: 'muted' as const }] },
          ],
        },
        {
          type: 'key-concepts' as const,
