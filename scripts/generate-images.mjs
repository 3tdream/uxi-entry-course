...
const IMAGES = [
  ...
  },
  {
    ...
    prompt: 'Clean educational empathy map poster in classic Dave Gray Xplane style on a white background. Layout: a large rectangular canvas divided by two diagonal lines (X-pattern) from corners forming four triangular quadrants — top "Think & Feel?", right "See?", bottom "Say & Do?", left "Hear?". Each quadrant is filled with several small colored sticky-note rectangles (yellow, pink, blue, mint, peach) — the stickies are blank shapes with no readable text. In the very center, a circle containing a friendly cartoon head silhouette of a person ("Susan" placeholder), a few small lines under it suggesting demographic info but unreadable. Below the main canvas, a horizontal strip split into two equal halves: left half labeled "Pain" with a few light-blue sticky notes, right half labeled "Gain" with a few light-pink sticky notes. The only readable text is the six section labels in plain English sans-serif: Think & Feel?, See?, Say & Do?, Hear?, Pain, Gain. Hand-drawn workshop poster aesthetic, clean modern UX education style, soft pastel palette, generous white space.',
  },
  {
