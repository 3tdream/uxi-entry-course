...
const IMAGES = [
  ...
  },
  {
    ...
    prompt: 'Clean educational poster with six perfectly identical rectangular cards arranged in a strict 3x2 grid. Each card is the EXACT same size with the same internal padding and centred content. Inside each card: a centred circular avatar of a flat character silhouette (head and shoulders), and below the avatar a single distinct tool icon. SIX DIFFERENT TOOL ICONS, one per card, in this order: 1) magnifying glass with tick (research), 2) wireframe-grid rectangle (UX), 3) painter palette with brush (UI), 4) lightning-bolt with arrow loop (product), 5) stacked-tokens / Lego-bricks (design system lead), 6) speech-bubble with quote-marks (writing). NO TEXT ANYWHERE on the image — pure visual icons only. All six characters drawn in the SAME flat illustration style with consistent body proportions, head sizes, and colour treatment. Uniform 16px gap between cards, generous margin around the grid. Flat minimalist UX education style, soft pastel palette (lavender, mint, peach, blue), white background, clean and symmetrical poster composition.',
  },
  {
    filename: 'meeting-02-double-diamond.webp',
    prompt: 'Canonical Double Diamond design process diagram, viewed straight-on, perfectly horizontal: two equal-size diamonds side by side, touching at their inner tips on the centre baseline. Left diamond filled in soft teal, right diamond filled in soft orange. NO TEXT ANYWHERE inside or around the diamonds. Above the entire composition four small visual icons aligned over the four diamond halves: a magnifying glass over the first half (left-diverge), a target with crosshair over the second (left-converge), three light-bulbs over the third (right-diverge), a rocket over the fourth (right-converge). Below the diamonds, two small minimal icons: a question-mark badge under the left diamond and a check-mark badge under the right diamond. Flat minimalist UX education style, white background, no characters, no readable letters or words anywhere — only shapes and icons.',
  },
  {
