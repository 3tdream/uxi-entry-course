...
const IMAGES = [
  ...
  },
  {
    ...
    prompt: 'Modern clean empathy-map template poster, edit.org / Miro style flat infographic, white background. Composition: a perfect 2x2 grid of four large rounded-rectangle panels arranged like a window — top-left panel soft peach, top-right panel mint-green, bottom-left panel lavender, bottom-right panel sky-blue. Each panel is identical in size, has a thin tinted border, contains 3 small blank rounded white cards stacked inside, and a small tinted icon in its top-left corner (top-left panel: brain icon, top-right panel: speech-bubble icon, bottom-left panel: heart icon, bottom-right panel: footprints icon). In the very center of the 2x2 grid, exactly at the four-panel intersection, sits a perfectly circular avatar with friendly cartoon person silhouette and headphones, white circle frame with subtle drop-shadow over the panels. Below the 2x2 grid, with a small gap, a single wide horizontal banner divided in half by a vertical line: left half soft warm-red tint with a small downward arrow icon and 2 blank rounded white cards, right half soft warm-green tint with a small upward arrow icon and 2 blank rounded white cards. Generous whitespace, gentle drop shadows on every card. ZERO TEXT in the image — no labels, no letters, no Cyrillic, no English, only icons and blank cards. Modern minimalist UX-design poster aesthetic.',
  },
  {
    filename: 'meeting-03-persona-card-filled.webp',
    prompt: 'Clean educational illustration of a fully filled-in UX Persona Card laid out as a single A4 portrait page on a wooden desk. Layout from top: header bar with "PERSONA CARD" label. Below: left column has a circular portrait of a friendly young man with headphones (around age 29, casual style — hint of a IT-professional vibe), and short labelled metadata blocks below it (Имя, Возраст, Роль). Right column: text blocks labelled Bio, Goals (3 bullets), Frustrations (3 bullets), Behaviour (device, frequency), Quote in italics, JTBD formula (3 lines: When / I want / so that). All text fields shown as readable placeholder lines with NO actual readable Cyrillic — just abstract line patterns indicating filled-in text. A small "A4" badge and a clean design-system grid hint behind the page. Flat minimalist UX education style, soft pastel palette (peach, lavender, mint), white card on warm desk, gentle shadow.',
  },
  {
  ...
  },
  {
    ...
    prompt: 'Clean educational diagram of a Jeff Patton User Story Map. Layout, top to bottom: (1) A horizontal "backbone" row of FIVE blue activity cards connected by small right-arrows between them, each card showing a single English label and icon: "Browse" (eye), "Search" (magnifier), "Compare" (balance-scale), "Buy" (shopping-cart), "Track" (location-pin). (2) Beneath each activity card, a vertical column of three small yellow sticky-note rectangles (blank yellow rectangles, no text on the stickies). (3) A bold horizontal dashed line cutting across right after the FIRST row of stickies, with a small left-side banner reading the English text "MVP" in plain sans-serif. (4) Below MVP, two more rows of yellow stickies labelled with small left-side banners "Release 2" and "Release 3" in English. Title at top of the whole composition: "User Story Map" in plain English sans-serif. Flat minimalist Agile-design illustration, soft pastel palette (blue cards, yellow stickies, white background), the only readable text on the image is the title, the five activity labels, and the three release banners (MVP, Release 2, Release 3) — no Cyrillic, no decorative text.',
  },
  {
